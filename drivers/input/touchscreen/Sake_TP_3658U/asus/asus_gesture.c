...
#define UDFPS                    "driver/udfps_event"
/*****************************************************************************
...
static ssize_t asus_proc_udfps_write(struct file *filp, const char *buff, size_t len, loff_t *off) {
	char messages[256];
	memset(messages, 0, sizeof(messages));
	...
	if ((strncmp(messages, "33", 2) == 0)) {
		if (fts_data->input_dev != NULL) {
			FTS_INFO("[UDFPS Event] Sending fingerprint udfps wake up event = %s\n", messages);
		...
		FTS_ERROR("[UDFPS Event] Received unknown event = %d\n", messages);
	}
...
static struct file_operations asus_gesture_proc_udfps_ops = {
	.write = asus_proc_udfps_write,
};
...
//Zenmotion
    ...
    proc_create(UDFPS, 0222, NULL, &asus_gesture_proc_udfps_ops);
